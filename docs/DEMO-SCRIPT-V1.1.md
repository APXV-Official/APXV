...
```powershell
python -c "from agents.voice import VoicePrivacyPipeline; ..."
```

**Say:** “Voice path: speech to text, same redaction engine, voice-redaction ZK circuit proves policy-bound redaction without exposing raw audio in the attestation bundle.”

**Show:** Transcript redaction summary, `voice-redaction` proof status, entity count ≥ 1.
...
**Say:** “Governance spine unchanged—rules, audit, capabilities. Privacy layers: redaction, encryption, dual-track ZK, now voice. Independent verify + signed ceremony transcript. Apache 2.0, runs offline.”

**End card:** Repo URL, `v1.1.0` tag, link to verifier bundle on Releases.
...
- Zoom terminal to full width; hide unrelated panels
