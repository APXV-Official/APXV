...
fn usage() -> ! {
    ...
    eprintln!();
    eprintln!("Usage:");
    eprintln!("  apxv-zk setup <circuit>");
    eprintln!("  apxv-zk prove <circuit> --inputs <inputs.json> [--out <proof.json>]");
    eprintln!("  apxv-zk verify <circuit> --inputs <inputs.json>");
    eprintln!("  apxv-zk hash-two <left_decimal> <right_decimal>");
    eprintln!("  apxv-zk hash-fields <decimal> [<decimal> ...]");
    eprintln!();
    eprintln!("Circuits:");
    for c in CIRCUITS {
        eprintln!("  {}", c);
...
fn run_prove(circuit: &str, inputs_path: &Path, out_path: Option<&Path>) {
    ...
    if !pk_path.exists() || !vk_path.exists() {
        eprintln!("No keys found for '{}'. Run: apxv-zk setup {}", circuit, circuit);
        std::process::exit(1);
    }
...
fn run_verify(circuit: &str, inputs_path: &Path) {
    ...
    if !vk_path.exists() {
        eprintln!("No verification key for '{}'. Run: apxv-zk setup {}", circuit, circuit);
        std::process::exit(1);
    }
...
fn main() {
    ...
    match args[1].as_str() {
        "setup" => {
            if args.len() < 3 {
                eprintln!("Usage: apxv-zk setup <circuit>");
                std::process::exit(1);
            }
        ...
        "prove" => {
            if args.len() < 5 || args[3] != "--inputs" {
                eprintln!("Usage: apxv-zk prove <circuit> --inputs <inputs.json> [--out <proof.json>]");
                std::process::exit(1);
            }
            let circuit = validate_circuit(&args[2]);
        ...
        "verify" => {
            if args.len() < 5 || args[3] != "--inputs" {
                eprintln!("Usage: apxv-zk verify <circuit> --inputs <inputs.json>");
                std::process::exit(1);
            }
            let circuit = validate_circuit(&args[2]);
            let inputs_path = Path::new(&args[4]);
        ...
        "hash-two" => {
            if args.len() < 4 {
                eprintln!("Usage: apxv-zk hash-two <left_decimal> <right_decimal>");
                std::process::exit(1);
            }
        ...
        "hash-fields" => {
            if args.len() < 3 {
                eprintln!("Usage: apxv-zk hash-fields <decimal> [<decimal> ...]");
                std::process::exit(1);
            }
