...
fn main() {
    ...
    if args.len() < 2 {
        ...
        eprintln!("Usage:");
        eprintln!("  apxv-circuits setup <redaction|rule-binding|pipeline>");
        eprintln!("  apxv-circuits prove <circuit> --inputs <inputs.json> [--out <proof.json>]");
        eprintln!("  apxv-circuits verify <circuit> --proof <proof_bundle.json>");
    ...
    match command.as_str() {
        "setup" => {
            if args.len() < 3 {
                eprintln!("Usage: apxv-circuits setup <redaction|rule-binding|pipeline>");
                std::process::exit(1);
            }
        ...
        "prove" => {
            if args.len() < 5 || args[3] != "--inputs" {
                eprintln!("Usage: apxv-circuits prove <redaction|rule-binding|pipeline> --inputs <file>");
                std::process::exit(1);
            }
        ...
        "verify" => {
            ...
            if args.len() < 5 || args[3] != "--proof" {
                eprintln!("Usage: apxv-circuits verify <redaction|rule-binding|pipeline> --proof <proof_bundle.json>");
                std::process::exit(1);
            }
