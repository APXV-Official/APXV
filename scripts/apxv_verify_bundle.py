...
Requires only:
  ...
  - The keys manifests (rust/apxv-circuits/keys/manifest.json, rust/apxv-zk/keys/entity-manifest.json) for VK integrity checks

Usage:
  python -m scripts.apxv_verify_bundle <proof_bundle.json> --circuit redaction
  python -m scripts.apxv_verify_bundle <attested_artifact.json> --circuit pipeline
"""
...
def _find_apx_circuits_binary() -> Path | None:
    candidates = [
        ...
        ROOT / "rust" / "target" / "debug" / "apxv-circuits.exe",
        ROOT / "rust" / "target" / "debug" / "apxv-circuits",
    ]
...
def main():
    ...
    parser.add_argument(
        "--check-binary",
        action="store_true",
        help="Only check that apxv-circuits binary is available",
    )
    ...
    if args.check_binary:
        if binary:
            ...
            return
        print("apxv-circuits binary not found. Build with: cargo build --release --manifest-path rust/Cargo.toml")
        sys.exit(1)
