...
Usage:
  ...
  python -m scripts.apx_serve --port 8741 --bind 127.0.0.1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="APX local API server (air-gapped)")
    ...
    if server.generated_key:
        ...
        print()
        print("Use header: Authorization: Bearer <key>")
        print("         or: X-APX-API-Key: <key>")
        print()

    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("\nShutting down APX server...")
        return 0
